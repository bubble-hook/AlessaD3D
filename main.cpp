...
HRESULT APIENTRY SetTexture_hook(LPDIRECT3DDEVICE9 pDevice, DWORD Sampler, IDirect3DBaseTexture9 *pTexture)
{
	...
	if (WTWall == 1)
	{
		PacthHook((void *)(DwRos + OFS_WTWall), (int*)(PBYTE)"\x66\x66\x66\xBF\x00", 5);
	}
	if (WTWall == 0)
	{
		PacthHook((void *)(DwRos + OFS_WTWall), (int*)(PBYTE)"\x00\x00\x00\xBF\x00", 5);
	}

	if (WTWater == 1)
	{
		PacthHook((void *)(DwRos + OFS_WTWater), (int*)(PBYTE)"\x91\x00\x00\x00\x8B", 5);
	}
	if (WTWater == 0)
	{
		PacthHook((void *)(DwRos + OFS_WTWater), (int*)(PBYTE)"\x90\x00\x00\x00\x8B", 5);
	}

	if (grave == 1)
	{
		PacthHook((void *)(DwRos + OFS_GRAVE), (int*)(PBYTE)"\x90\x90\x90\x90\x90\x90\x90\x90", 8);
	}
	if (grave == 0)
	{
		PacthHook((void *)(DwRos + OFS_GRAVE), (int*)(PBYTE)"\xF3\x0F\x11\x87\xE0\x00\x00\x00", 8);
	}
	if (speedhack == 1)
	{
		PacthHook((void *)(DwRos + OFS_SPEED), (int*)(PBYTE)"\x00\x00\xC8\x42\x00", 5);		
	}
	if (speedhack == 0)
	{
		PacthHook((void *)(DwRos + OFS_SPEED), (int*)(PBYTE)"\x00\x00\x70\x42\x00", 5);
	}
...
HRESULT APIENTRY Present_hook(IDirect3DDevice9* pDevice, const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	if (ShowMenu)
		//draw background
		DrawBox(pDevice, 71.0f, 86.0f, 200.0f, 355.0f, D3DCOLOR_ARGB(250, 128, 128, 128));//180 = up/down, 200 = left/right
