...
#define OFS_NOFOG		0xC37FFE
#define OFS_GRAVE		0x544578
#define OFS_WTWall		0x175A6B8
#define OFS_WTWater		0x39665C
#define OFS_SPEED		0x1763A30
...
int autoshoot = 0;				//autoshoot settings
...
void SaveCfg()
{
	...
	fout << "WTWall " << WTWall << endl;
	fout << "WTWater " << WTWater << endl;
	fout << "grave " << grave << endl;
	fout << "speedhack " << speedhack << endl;
	fout.close();
}
...
char *opt_nothing[] = { "INTERNAL" };
char *opt_nothing2[] = { " " };
char *opt_nothing3[] = { ".alessa#0008" };
...
void DrawMenu(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (ShowMenu)
	{
		...
		Current = 1;
		AddItem(pDevice, " AlessaPrivate -", nofiture, opt_nothing, 1);
		AddItem(pDevice, " Discord", nofiture, opt_nothing3, 1);
		AddItem(pDevice, " ", nofiture, opt_nothing2, 1);
		AddItem(pDevice, " - Visual -", nofiture, opt_nothing2, 1);
		AddItem(pDevice, " WallHack", wallhack, opt_WhChams, 1);
		AddItem(pDevice, " ESP BOX", esp, opt_ZeroTen, 10);
		AddItem(pDevice, " ESP Line", line, opt_ZeroTen, 10);
		AddItem(pDevice, " ", nofiture, opt_nothing2, 1);
		AddItem(pDevice, " - AIMBOT -", nofiture, opt_nothing2, 1);
		AddItem(pDevice, " Aimbot", aimbot, opt_OnOff, 2);
		AddItem(pDevice, " Aimkey", aimkey, opt_Keys, 4);
		AddItem(pDevice, " Aimsens", aimsens, opt_ZeroTen, 10);
		AddItem(pDevice, " Aimfov", aimfov, opt_aimfov, 7);
		AddItem(pDevice, " Aimheight", aimheight, opt_ZeroTen, 10);
		AddItem(pDevice, " Autoshoot", autoshoot, opt_autoshoot, 1);
		AddItem(pDevice, " ", nofiture, opt_nothing2, 1);
		AddItem(pDevice, " - MISC -", nofiture, opt_nothing2, 1);
		AddItem(pDevice, " No Grass", nograss, opt_OnOff, 1);
		AddItem(pDevice, " No Fog", nofog, opt_OnOff, 1);
		AddItem(pDevice, " SpeedHack", speedhack, opt_OnOff, 1);
		AddItem(pDevice, " WTWall", WTWall, opt_OnOff, 1);
		AddItem(pDevice, " WTWater", WTWater, opt_OnOff, 1);
		AddItem(pDevice, " ProneHack", grave, opt_OnOff, 1);
